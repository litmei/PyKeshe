...
class Painter:
	...
	def st_draw_line_img(config):
		...
		my_font = matplotlib.font_manager.FontProperties(family="SimSun", size=10)
		...
		while True:
			...
			plt.grid()  # 网格
			plt.title("({0}){1}".format(Painter.__abc[count], Painter.__data_label[0][ii]), size=20, y=-0.15)  # 标题
			plt.xlabel(Painter.__x_label, x=0.99)  # x轴标签
			plt.ylabel(Painter.__data_label[1][ii], y=0.99)  # y轴标签
			count += 1
